...
<template>
  <UContainer class="max-w-full py-6">
      ...
      <template #body>
        <div class="flex flex-wrap items-center justify-between gap-1.5 mb-4 max-w-full">
          ...
          <div class="flex flex-wrap items-center gap-1.5">
            ...
            ]" placeholder="Filter category" class="min-w-28" />
            <UButton label="Add Product" color="primary" icon="i-lucide-plus" @click="showAddModal = true" />
        ...
        <UTable ref="table" v-model:column-filters="columnFilters" v-model:column-visibility="columnVisibility"
          ...
          :columns="columns" :loading="isFetching" :ui="{
        ...
        <div class="flex items-center justify-between gap-3 border-t border-default pt-4 mt-auto">
          ...
          <UPagination :default-page="(table?.tableApi?.getState().pagination.pageIndex || 0) + 1"
            :items-per-page="table?.tableApi?.getState().pagination.pageSize" :total="totalProducts"
            @update:page="(p: number) => table?.tableApi?.setPageIndex(p - 1)" />
        </div>
      </template>
    </UDashboardPanel>

    <UModal title="Add New Product" v-model:open="showAddModal">
      <template #body>
        <UForm :schema="schema" :state="state" class="p-4" @submit="onSubmit">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <UFormField label="Name" name="name" required class="col-span-1">
              <UInput v-model="state.name" placeholder="Product name" />
            </UFormField>
            <UFormField label="Price" name="price" required class="col-span-1">
              <UInput v-model.number="state.price" type="number" min="0" placeholder="Price" />
            </UFormField>
            <UFormField label="Stock" name="stock" required class="col-span-1">
              <UInput v-model.number="state.stock" type="number" min="0" placeholder="Stock" />
            </UFormField>
            <UFormField label="Category" name="category" required class="col-span-1">
              <USelect v-model="state.category" :items="categories" placeholder="Select category" />
            </UFormField>
            <UFormField label="Brand" name="brand" class="col-span-1">
              <UInput v-model="state.brand" placeholder="Brand name" />
            </UFormField>
            <UFormField label="Description" name="description" class="col-span-1 md:col-span-2">
              <UInput v-model="state.description" placeholder="Description" />
            </UFormField>
            <UFormField label="Product Images" name="images" required class="col-span-1 md:col-span-2">
    ...
    <UModal title="Edit Product" v-model:open="showEditModal">
      <template #body>
        <UForm :schema="schema" :state="editState" class="p-4" @submit="onUpdate">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <UFormField label="Name" name="name" required class="col-span-1">
            ...
            <UFormField label="Category" name="category" required class="col-span-1">
              <USelect v-model="editState.category" :items="categories" placeholder="Select category" />
            </UFormField>
            <UFormField label="Brand" name="brand" class="col-span-1">
            ...
            <UFormField label="Product Images" name="images" class="col-span-1 md:col-span-2">
              <input type="file" multiple accept="image/*" @change="handleImageChange"
                class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100" />
              <p class="mt-1 text-sm text-gray-500">Select one or more images (max 5MB each)</p>
            </UFormField>
